    ...
    <v-container fill-height>
      <v-layout column>
        ...
        <v-flex shrink class="white pa-2">
          ...
          <v-btn v-if="step < 4" large color="teal lighten-1 white--text" @click="step += 1" :disabled="disableNext">Next</v-btn>
          <v-btn v-else large color="teal lighten-1 white--text" @click="submit" :loading="loading" :disabled="loading || disableNext">Submit</v-btn>
          {{ disableNext ? '* Please fill all the required fields.' : '' }}
        </v-flex>
      </v-layout>
    </v-container>
...
export default {
  ...
  computed: {
    disableNext () {
      ...
      switch (this.step) {
        case 1:
          return !sex || !age_group || !organization
          break
        case 2:
